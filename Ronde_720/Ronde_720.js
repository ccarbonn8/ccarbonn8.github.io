...
function buildArcherTable(containerId) {
  ...
  for (let line = 1; line <= 12; line++) {
    ...
    for (let i = 1; i <= 6; i++) {
      tr.appendChild(makeArrowCell(`arrow_${arrowId++}`));
    }
...
function moveNextInput() {
  ...
  const inputs = [
    ...Array.from({length: 72}, (_, i) => currentArcher.querySelector('#arrow_' + (i + 1))),
  ];
...
function updateSumsAndCumul() {
  ...
  if (!currentArcher) return; // Safety: no visible archer
	...
	for (let line = 1; line <= 12; line++) {
	  let el1 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 1));
	  let el2 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 2));
	  let el3 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 3));
	  let el4 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 4));
	  let el5 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 5));
	  let el6 = currentArcher.querySelector('#arrow_' + ((line - 1) * 6 + 6));

	  let val1 = parseScore(el1?.value || "");
	  let val2 = parseScore(el2?.value || "");
	  let val3 = parseScore(el3?.value || "");
	  let val4 = parseScore(el4?.value || "");
	  let val5 = parseScore(el5?.value || "");
	  let val6 = parseScore(el6?.value || "");
...
function focusFirstEmptyInput() {
  ...
  // First left side arrows (1 to 72)
  for (let i = 1; i <= 72; i++) {
    const input = currentArcher.querySelector(`#arrow_${i}`);
...
function fillGoogleForm() {
	...
	for (let i = 1; i <= 4; i++) {
	  ...
	  const cumulative1to12 = calculateCumulative(archer.scores); // lines 1 to 12
	  ...
	  if(archer.name === '' || countEmpty > 0) {
		...
		previewText += `Cumul 12: ${cumulative1to12}\n`;
		...
		formData.append('entry.204468612', cumulative1to12);
...
function calculateCumulative(scores) {
  let sum = 0;
  for (let j = 0; j < 72; j++) {
    const el1 = scores[j];
